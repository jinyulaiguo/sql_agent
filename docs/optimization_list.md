...
| **问题3** | 检索结果无二次排序 |
| **方案** | 引入 Cross-Encoder Re-ranking 模型对 Top-K 结果精排 |
...
- [ ] 将 ReAct 循环改为**确定性 Pipeline**（RAG→Schema→LLM 生成 SQL→执行→总结）
- [ ] 仅在 SQL 执行报错时启用 LLM 重试推理（最多 2 次）
- [ ] LLM 调用从 4 次降至 1-2 次

#### 中期（下个版本）—— 检索增强

- [ ] RAG 索引文档去除 DDL 噪音，增加业务语义描述
- [ ] Embedding 模型切换为 `bge-base-zh-v1.5` 或多语言模型
- [ ] 引入 BM25 + 向量混合检索
- [ ] 支持多轮对话上下文
